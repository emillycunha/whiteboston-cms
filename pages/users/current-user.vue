  ...
  try {
    ...
    sidebarOrder.value =
      collections.value.filter((collection) =>
        (dashboardPreferences.sidebarOrder || []).includes(collection.slug)
      ) || [];
...
const saveSidebarOrder = async () => {
  try {
    const preferencesChanged =
      ...
      JSON.stringify(sidebarOrder.value.map((collection) => collection.slug));
    ...
    if (preferencesChanged) {
      ...
      if (success) {
        initialPreferences.value.sidebarOrder = JSON.parse(
          JSON.stringify(
            sidebarOrder.value.map((collection) => collection.slug)
          )
...
const toggleSidebarOrder = (collection) => {
  if (isInSidebarOrder(collection)) {
    ...
    sidebarOrder.value = sidebarOrder.value.filter(
...
const saveSettings = async () => {
  try {
    const collectionsChanged =
      JSON.stringify(initialCollections.value) !==
      JSON.stringify(collections.value);

    const preferencesChanged =
      JSON.stringify(initialPreferences.value) !==
      JSON.stringify(selectedStats.value);
